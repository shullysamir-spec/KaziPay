      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
        ...
        <div className="p-4 bg-[#1F3864] text-white flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#BF9000] text-[#1F3864] flex items-center justify-center font-bold">
          ...
          <button onClick={onClose} className="p-1 text-[#BF9000] hover:text-white rounded transition">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-2.5 max-h-[60vh] overflow-y-auto">
          {shortcuts.map((s, idx) => (
            ...
            >
              ...
              <kbd className="px-2.5 py-1 text-[11px] font-mono font-bold text-[#1F3864] dark:text-blue-300 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg shadow-2xs">
                {s.key}
              </kbd>
            </div>
          ))}
        ...
        <div className="p-3.5 bg-slate-50 dark:bg-slate-800/60 border-t border-slate-200 dark:border-slate-800 text-center flex items-center justify-between text-[11px] text-slate-500">
          ...
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-[#1F3864] text-white font-bold rounded-lg text-xs hover:bg-[#152747] transition"
          >
            {t.close}
          </button>
        </div>
